...
(function () {
  ...
  function lockIfNeeded(onOk) {
    ...
    ov.innerHTML = `
      <div class="lock-card">
        <img src="logo.png" alt="ReelVault" class="lock-logo"/>
        <h1>ReelVault</h1>
  ...
  function buildShell(page) {
    ...
    top.innerHTML = `
      <a class="brand" href="index.html">
        <img src="logo.png" alt="logo"/><span>Reel<b class="grad-text">Vault</b></span>
      </a>
...
/* ============================================================
   v8.2 — sidebar section labels (Menu / Tools) for brief spacing
   ============================================================ */
(function () {
  "use strict";
  if (!window.RVUI) return;
  const orig = RVUI.init.bind(RVUI);
  RVUI.init = function (page) {
    orig(page);
    try {
      const side = document.querySelector(".sidebar");
      if (!side || side.querySelector(".side-label")) return;
      const first = side.querySelector(".side-link[href]");
      const tools = side.querySelector(".js-xl-side");
      const mk = (t) => { const d = document.createElement("div"); d.className = "side-label"; d.textContent = t; return d; };
      if (first) side.insertBefore(mk("Menu"), first);
      if (tools) side.insertBefore(mk("Tools"), tools);
    } catch (_) {}
  };
})();

/* ============================================================
   v8.3 — 💬 RVLeads (influencer DM store) + 📲 INSTAGRAM SHARE TARGET
   - Lead: sirf localStorage (rv_leads) — Sheet/backup se alag nahi girta,
     backup JSON mein shamil. Kabhi bhi add/edit karo.
   - Share: Android PWA "Share → ReelVault" se ?text/?url aata hai →
     unlock ke baad Quick Add khulta hai, link pehle se bhara hota hai.
   Pipeline ke code ko haath nahi lagta.
   ============================================================ */
(function () {
  "use strict";

  /* ---------------- 💬 LEAD STORE ---------------- */
  const LS_LEADS = "rv_leads";
  function readLeads() { try { return JSON.parse(localStorage.getItem(LS_LEADS) || "{}") || {}; } catch (_) { return {}; } }
  function writeLeads(o) { try { localStorage.setItem(LS_LEADS, JSON.stringify(o)); } catch (_) {} }
  window.RVLeads = {
    get(sr) { const l = readLeads()[String(sr)]; return l && (l.msg || l.link) ? l : null; },
    has(sr) { return !!this.get(sr); },
    set(sr, lead) {
      const o = readLeads();
      if (lead && (lead.msg || lead.link)) o[String(sr)] = { msg: lead.msg || "", link: lead.link || "", at: new Date().toISOString() };
      else delete o[String(sr)];
      writeLeads(o);
    },
  };
  ...
  /* URL turant saaf — refresh pe dobara na khule; link session mein safe */
  (function stashShare() {
    const u = extractSharedUrl();
    if (!u) return;
    try { sessionStorage.setItem("rv_pending_share", JSON.stringify({ url: u, at: Date.now() })); } catch (_) {}
    try { history.replaceState(null, "", location.pathname); } catch (_) {}
  })();

  function peekPendingShare() {
    try {
      const d = JSON.parse(sessionStorage.getItem("rv_pending_share") || "null");
      if (d && d.url && Date.now() - (d.at || 0) < 10 * 60 * 1000) return d.url;
    } catch (_) {}
    return "";
  }

  function fillSharedLink(wrap, url) {
    const inp = wrap && wrap.querySelector("#qa-link");
    if (!inp) return;
    inp.value = url;
    inp.dispatchEvent(new Event("input", { bubbles: true }));
  }

  function openSharedQuickAdd(url) {
    try { sessionStorage.removeItem("rv_pending_share"); } catch (_) {}
    RVUI.openQuickAdd();
    const wrap = document.querySelector(".modal-wrap");
    if (!wrap) return;
    /* 📲 banner — user ko pata chale link Instagram se aaya */
    const head = wrap.querySelector(".qa-head");
    if (head && !head.querySelector(".share-banner")) {
      const b = document.createElement("div");
      b.className = "share-banner";
      b.innerHTML = "📲 <b>Instagram se direct share</b> — link aa gaya! Topic/rating check karke bas <b>ADD VIDEO</b> dabao ✨";
      head.appendChild(b);
    }
    const submit = wrap.querySelector("#qa-form button[type=submit]");
    if (submit) submit.classList.add("share-pulse");
    /* clipboard auto-fill race ko harane ke liye 3 baar likho — aakhri likhai jeet-ti hai */
    [60, 450, 950].forEach((ms) => setTimeout(() => fillSharedLink(wrap, url), ms));
    setTimeout(() => { const i = wrap.querySelector("#qa-link"); if (i && i.value !== url) fillSharedLink(wrap, url); }, 1600);
  }

  /* RVUI.init wrap — shell banne (unlock) ka wait karke share kho lo */
  const prevInit83 = RVUI.init;
  RVUI.init = function (page) {
    prevInit83(page);
    const sharedUrl = peekPendingShare();
    if (!sharedUrl || page !== "dashboard") return;
    let tries = 0;
    const iv = setInterval(() => {
      tries++;
      const shellReady = !!document.querySelector(".js-quickadd") && !document.querySelector(".lock-screen");
      if (shellReady) {
        clearInterval(iv);
        setTimeout(() => openSharedQuickAdd(sharedUrl), 350);
      } else if (tries > 40) clearInterval(iv); /* 16s baad give up */
    }, 400);
  };
})();
