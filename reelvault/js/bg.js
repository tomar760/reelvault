...
(function () {
  ...
  function makeOrbs() {
    const nodes = [];
    for (let i = 0; i < 26; i++) nodes.push({ x: R(0, W), y: R(0, H), r: R(2, 5), vx: R(-0.25, 0.25), vy: R(-0.2, 0.2), hue: R(210, 265) });
    return () => {
  ...
  function makeStrips() {
    const frames = [];
    for (let i = 0; i < 16; i++) frames.push({ x: R(0, W), y: R(0, H + 200), w: R(70, 130), h: R(44, 78), sp: R(0.25, 0.7), sway: R(0, 6.28), sa: R(0.15, 0.5) });
    return () => {
      for (const f of frames) {
        f.y -= f.sp; f.sway += 0.008;
        if (f.y < -120) { f.y = H + 120; f.x = R(0, W); }
        const x = f.x + Math.sin(f.sway) * 14;
        ctx.strokeStyle = `rgba(45, 164, 126, ${f.sa})`;
        ctx.lineWidth = 1.6;
        ctx.beginPath();
        const r = 9;
        ctx.roundRect(x, f.y, f.w, f.h, r); ctx.stroke();
        ctx.fillStyle = `rgba(45, 164, 126, ${f.sa * 0.14})`;
        ctx.beginPath(); ctx.roundRect(x, f.y, f.w, f.h, r); ctx.fill();
        // play triangle
        ctx.fillStyle = `rgba(45, 164, 126, ${f.sa * 0.8})`;
  ...
  function makeShards() {
    const sh = [];
    for (let i = 0; i < 12; i++) sh.push({ x: R(0, W), y: R(0, H), s: R(26, 84), rot: R(0, 6.28), vr: R(-0.004, 0.004), vx: R(-0.15, 0.15), vy: R(-0.1, 0.1), a: R(0.05, 0.16) });
    return () => {
      for (const s of sh) {
        ...
        g.addColorStop(0, `rgba(130, 80, 223, ${s.a})`);
        g.addColorStop(1, `rgba(88, 166, 255, ${s.a * 0.7})`);
        ctx.fillStyle = g;
        ctx.beginPath(); ctx.moveTo(0, -s.s); ctx.lineTo(s.s * 0.7, s.s * 0.5); ctx.lineTo(-s.s * 0.7, s.s * 0.5); ctx.closePath(); ctx.fill();
        ctx.strokeStyle = `rgba(150, 110, 240, ${s.a + 0.05})`; ctx.lineWidth = 1.2; ctx.stroke();
        ctx.restore();
      }
    };
  }
  ...
  function makeWaves() {
    ...
    return () => {
      ...
      for (let i = 0; i < N; i++) {
        ...
        ctx.fillStyle = `rgba(191, 117, 0, ${0.05 + bars[i] * 0.05})`;
      ...
      for (let k = 0; k < 3; k++) {
        ...
        ctx.strokeStyle = `rgba(217, 119, 6, ${0.10 - k * 0.02})`;
  ...
  function makeRings() {
    ...
    return () => {
      ...
      for (const r of rings) {
        ...
        ctx.strokeStyle = `rgba(207, 34, 46, ${r.a * fade})`;
        ctx.lineWidth = 1.6;
        ctx.beginPath(); ctx.arc(r.x, r.y, r.r, 0, 7); ctx.stroke();
        ctx.strokeStyle = `rgba(120, 130, 145, ${r.a * fade * 0.7})`;
  ...
  function makeAurora() {
    return () => {
      for (let k = 0; k < 4; k++) {
        ...
        g.addColorStop(0.5, `hsla(${215 + k * 10}, 60%, 58%, ${0.05 + 0.02 * Math.sin(t / 60 + k)})`);
