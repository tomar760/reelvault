...
(function () {
  ...
  /* v8.3 perf: phone/low-power pe LITE mode — kam particles, DPR 1, 30fps cap.
     (rv_perf=1 se force-lite; rv_anim_bg=off se pura off — Settings toggle) */
  let DPR = Math.min(window.devicePixelRatio || 1, 2), LITE = false, fr = 0;
  const cnt = (n) => (LITE ? Math.max(3, Math.round(n * 0.35)) : n);
  ...
  function makeOrbs() {
    const nodes = [];
    for (let i = 0; i < cnt(26); i++) nodes.push({ x: R(0, W), y: R(0, H), r: R(2, 5), vx: R(-0.25, 0.25), vy: R(-0.2, 0.2), hue: CANDY[i % 3] + R(-12, 12) });
    return () => {
  ...
  function makeStrips() {
    const frames = [];
    for (let i = 0; i < cnt(16); i++) frames.push({ x: R(0, W), y: R(0, H + 200), w: R(70, 130), h: R(44, 78), sp: R(0.25, 0.7), sway: R(0, 6.28), sa: R(0.07, 0.22), hue: CANDY[i % 3] });
    return () => {
  ...
  function makeShards() {
    const sh = [];
    for (let i = 0; i < cnt(12); i++) sh.push({ x: R(0, W), y: R(0, H), s: R(26, 84), rot: R(0, 6.28), vr: R(-0.004, 0.004), vx: R(-0.15, 0.15), vy: R(-0.1, 0.1), a: R(0.05, 0.16), hue: CANDY[i % 3] });
    return () => {
      for (const s of sh) {
  ...
  function makeWaves() {
    const N = cnt(42), bars = new Array(N).fill(0).map(() => R(0.2, 1));
    return () => {
      const bw = W / N;
      for (let i = 0; i < N; i++) {
  ...
  function makeRings() {
    ...
    for (let i = 0; i < cnt(5); i++) spawn();
    return () => {
      if (Math.random() < 0.02 && rings.length < cnt(9)) spawn();
  ...
  function frame() {
    if (!running) return;
    fr++;
    /* LITE: har doosra frame skip — ~30fps, aadha CPU load */
    if (!LITE || fr % 2 === 0) {
      t++;
  ...
  window.RV_BG = {
    start(name) {
      ...
      LITE = window.innerWidth < 760 || (navigator.hardwareConcurrency || 8) <= 4 || localStorage.getItem("rv_perf") === "1";
